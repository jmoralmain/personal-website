...
| Route network  | One connected **route** — quiet and understated, not a literal highway: a single muted stone ribbon (`THEME.road`, ~0.11 world units, no lane markings), re-projected onto the sphere so it conforms to the surface, laid just above the territory caps and under the photos. A **spine** loops around the whole globe through every region center; **branches** leave the spine perpendicularly (toward the open poles, away from neighbor regions) and curve through each region's photos along a single constant-curvature arc — which never crosses itself, so the routes don't tangle. Opaque (z-buffered) with frustum culling off, so it stays visible but never flickers/pops as the globe rotates. Photos sit ~22° apart (clearing the ~19° tile width) so the route reads between them. Low-contrast and slim so it suggests a path without overpowering the photos or the lime accent. Core THREE only |
