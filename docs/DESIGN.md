...
| Trail network  | One connected set of **wide solid trails** worn into the terrain — ribbon meshes (flat strips ~0.13 world units, re-projected onto the sphere so they conform to the surface), laid just above the territory caps and under the photos. A path you follow, not a survey line. A neutral **ash spine** (`THEME.ash`, ~72% opacity) loops around the whole globe through every region center; **region-tinted branches** (~80%) flow off it through each region's photos; sparse **forks** (~50%, thinning to a fade) peel off and trail away. Photos sit ~28° apart (clearing the ~19° tile width) so the trail reads between them. Core THREE only — no Line2 addon |
