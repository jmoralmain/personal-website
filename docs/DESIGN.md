...
| Road network   | One connected **road** drawn to read like real pavement: solid asphalt (`THEME.road`, ~0.2 world units wide), two painted **edge lines** and a **dashed center line** (white `THEME.vellum`, lifted a hair above the asphalt). Ribbon meshes (flat strips re-projected onto the sphere so they conform to the surface), laid just above the territory caps and under the photos. A **spine** loops around the whole globe through every region center; **branches** of the same look flow off it through each region's photos. Rendered opaque (z-buffered, not transparency-sorted) with frustum culling off, so it reads as real road and never flickers/pops as the globe rotates. Photos sit ~28° apart (clearing the ~19° tile width) so the road reads between them. Core THREE only |
