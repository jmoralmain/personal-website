...
| Road network   | One connected **road** drawn to read like real pavement: dark asphalt (`THEME.road`), two painted **edge lines** and a **dashed center line** (white `THEME.vellum`, lifted a hair above the asphalt). Ribbon meshes (flat strips ~0.2 world units, re-projected onto the sphere so they conform to the surface), laid just above the territory caps and under the photos. A **spine** loops around the whole globe through every region center; **branches** leave the spine perpendicularly (toward the open poles, away from neighbor regions) and curve through each region's photos along a single constant-curvature arc — which never crosses itself, so the roads don't tangle. Rendered opaque (z-buffered) with frustum culling off, so it reads as real road and never flickers/pops as the globe rotates. Photos sit ~22° apart (clearing the ~19° tile width) so the road reads between them. Core THREE only |
