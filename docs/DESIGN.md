...
| Road network   | One connected **road** — a single solid asphalt color (`THEME.road`), opaque and wide (~0.2 world units), that you follow across the globe. Ribbon meshes (flat strips re-projected onto the sphere so they conform to the surface), laid just above the territory caps and under the photos. A **spine** loops around the whole globe through every region center; **branches** of the same width and color flow off it through each region's photos. Rendered opaque (z-buffered, not transparency-sorted) and with frustum culling off, so it reads as real pavement and never flickers/pops as the globe rotates. Photos sit ~28° apart (clearing the ~19° tile width) so the road reads between them. Core THREE only |
